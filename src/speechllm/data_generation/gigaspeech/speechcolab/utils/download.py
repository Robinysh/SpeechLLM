...
def download_from_http(local_filename, url, show_progress_bar=False):
    ...
    with open(local_filename, "wb") as f:
        pbar = (
            tqdm(range(length), unit="B", unit_scale=True, unit_divisor=1024)
...
def download_from_ftp(
    local_filename, host, remote_path, username=None, password=None, port=None
):
    ftp = ftplib.FTP()
...
def download(local_filename, url, show_progress_bar=False):
    ...
    if url.startswith("http://") or url.startswith("https://"):
        ...
        return download_from_ftp(
            local_filename,
            url_info.hostname,
            url_info.path,
            url_info.username,
            url_info.password.replace("<SLASH>", "/"),
            url_info.port,
        )
    else:
