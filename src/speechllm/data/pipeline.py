...
def hf_text_interface_pipeline(
    database,
    num_workers=16,
    cache_dir=None,
    use_cache=True,
    ...
    ds = ds.map(
        lambda x: {
            "infer_prompt": prompter.generate_template_with_text_interface(
                x["input_tokens"]
            )
        },
        batched=False,
    ...
    ds = ds.map(
        lambda x: {
            "prompt": prompter.generate_template_with_text_interface(
                x["input_tokens"], x["output_tokens"], x["output_transcript"]
            )
        },
        batched=False,
        num_proc=num_workers,
