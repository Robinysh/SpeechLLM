...
class ModuleFusedSDPA(torch.nn.Module):
    ...
    def forward(
        self, query, key, value, attn_mask, dropout_p, is_casual, scale, softmax_mode
    ):
        return self._hpu_kernel_fsdpa.apply(
            query, key, value, attn_mask, dropout_p, is_casual, scale, softmax_mode
        )
...
class GaudiLlamaAttention(LlamaAttention):
    def __init__(self, config: LlamaConfig, layer_idx: Optional[int] = None):
        ...
        self.fused_scaled_dot_product_attention = (
            ModuleFusedSDPA(FusedSDPA) if FusedSDPA else None
        )
        self.inp_seq_len = -1
    ...
    def pre_attn_forward(
        ...
        **kwargs,
        ...
        if past_key_value is not None:
            if token_idx is None:
                ...
                if reuse_cache and not isinstance(past_key_value[0], torch.Tensor):
                    ...
                    if (
                        num_virtual_tokens is not None
        ...
        if use_cache:
            ...
            if reuse_cache:
                if past_key_value is not None and isinstance(
                    past_key_value[0], torch.Tensor
                ):
        ...
        if use_flash_attention and FusedSDPA:
            ...
            attn_weights = (
                self.matmul_qk(query_states, key_states.transpose(-2, -1))
                * self.norm_factor
            )
            ...
            if attn_softmax_bf16:
                attn_weights = torch.nn.functional.softmax(
                    attn_weights, dim=-1, dtype=query_states.dtype
                )
            else:
...
class GaudiLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def pre_attn(
        ...
        hidden_states, attn_weights, present_key_value = (
            self.self_attn.pre_attn_forward(
                hidden_states,
                attention_mask,
                position_ids,
                past_key_value,
...
class GaudiLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = torch.nn.ModuleList(
            [
                GaudiLlamaDecoderLayer(config, layer_idx)
    ...
    def forward(
        ...
        if (
            hasattr(self.config, "use_fused_rope")
            and self.config.use_fused_rope is False
        ):
            global has_fused_rope
            has_fused_rope = False
        if (
            hasattr(self.config, "use_fused_rms_norm")
            and self.config.use_fused_rms_norm is False
        ):
...
class GaudiLlamaForCausalLM(LlamaForCausalLM):
    ...
    def __init__(self, config):
        ...
        self.model = GaudiLlamaModel(config)
        self.vocab_size = config.vocab_size
    ...
    def forward(
        ...
        if (
            hasattr(self.generation_config, "use_fused_rope")
            and self.generation_config.use_fused_rope is False
        ):
            global has_fused_rope
            has_fused_rope = False
        ...
        model_inputs.update(
            {
                ...
                "flash_attention_causal_mask": kwargs.get(
                    "flash_attention_causal_mask"
                ),
                "flash_attention_fast_softmax": kwargs.get(
                    "flash_attention_fast_softmax"
                ),
    ...
    if q.device.type == "hpu" and has_fused_rope:
        ...
        if k.dtype == torch.bfloat16:
            return FusedRoPE.apply(
                q,
                cos.unsqueeze(0).unsqueeze(0).clone(),
                sin.unsqueeze(0).unsqueeze(0).clone(),
                position_ids,
            ), FusedRoPE.apply(
                k,
