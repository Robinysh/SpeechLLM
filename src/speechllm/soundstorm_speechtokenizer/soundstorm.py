...
@beartype
def get_mask_subset_prob(mask: Tensor, prob: Union[float, Tensor], min_mask: int = 0):
...
class Conformer(nn.Module):
    def __init__(
        ...
        *,
        ...
        assert not (
            t5_rel_pos_bias and attn_flash
        ), "flash attention is not compatible with learned bias"
        ...
        self.rel_pos_bias = (
            T5RelativePositionBias(dim_head**0.5, heads=heads)
            if t5_rel_pos_bias
            else None
        )
    ...
    def forward(self, x, mask=None):
        ...
        for block in self.layers:
            x = block(x, mask=mask, rotary_emb=rotary_emb, attn_bias=attn_bias)
...
class ConformerWrapper(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.embedding_proj = (
            nn.Sequential(nn.Linear(dim * grouped_quantizers, dim), nn.LayerNorm(dim))
            if grouped_quantizers > 1
            else nn.Identity()
        )
        ...
        self.register_buffer(
            "mask_tokens",
            self.quantizer_offsets + num_codes_with_mask,
            persistent=False,
        )
    ...
    def forward(
        ...
        *,
        ...
        assert divisible_by(
            n, g * q
        ), "sequence must be divisible by number of quantizers"
...
class SoundStorm(nn.Module):
    ...
    def get_condition(self, token_ids, length=None):
        ...
        cond_tokens = semantic_tokens.masked_fill(~rearrange(mask, "... -> ... 1"), 0.0)
        if exists(length):
            cond_length = cond_tokens.size(-2)
            if cond_length < length:
                cond_length = F.pad(
                    cond_tokens, (0, 0, 0, length - cond_length), value=0.0
                )
    ...
    def generate(
        ...
        mask = torch.ones(
            (batch_size, semantic_tokens.size(-1), self.num_quantizers), device=device
        )
        ...
        if exists(prompt_tokens):
            ...
            mask = torch.cat(
                [
                    torch.zeros_like(
                        prompt_acoustic_tokens, dtype=torch.bool, device=device
                    ),
                    mask.bool(),
                ],
                axis=1,
        ...
        seq_mask_with_quantizer = repeat(
            seq_mask, "b n -> b (n q)", q=self.num_quantizers
        )
        ...
        for q in range(self.num_quantizers):
            all_mask_num_tokens = (
                ...
                else torch.zeros((1, batch_size), dtype=torch.long, device=device)
            )
            for i, mask_num_tokens in enumerate(all_mask_num_tokens):
                ...
                if greedy and (
                    q >= num_full_sampling_levels or (mask_num_tokens == 0).all()
                ):
                ...
                if q >= num_full_sampling_levels:
                    masked[:, :, q:] = rearrange(
                        sampled_ids, "b (n q) -> b n q", q=self.num_quantizers
                    )[:, :, q:]
                    mask[:, :, q] = False
                    continue
                ...
                masked = torch.where(
                    mask,
                    rearrange(sampled_ids, "b (n q) -> b n q", q=self.num_quantizers),
                    masked,
                )
    ...
    def forward(self, x, cond_ids, mask=None, generator_sample_temperature=1, **kwargs):
        ...
        upper_quantizers_mask = torch.full(
            (b, n, self.num_quantizers - q - 1), True, device=device
        )
        ...
        logits = self.net(masked, mask=seq_mask, cond=cond_tokens, **kwargs)
